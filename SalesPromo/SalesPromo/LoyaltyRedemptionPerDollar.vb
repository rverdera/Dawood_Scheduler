...
Public Class LoyaltyRedemptionPerDollar
    ...
    Private Sub SaveLoyalty()
        Try
            ...
            If txtPoints.Text = String.Empty Then
                ...
                Return
            End If
            objDO.ExecuteSQL("Delete from LoyaltyRedPerDollar where MemberType=" & objDO.SafeSQL(txtCode.Text))
            Dim strSql As String
            strSql = "Insert into LoyaltyRedPerDollar(MemberType, Points, DTG) Values (" & objDO.SafeSQL(txtCode.Text) & "," & txtPoints.Text & "," & objDO.SafeSQL(Format(DateTime.Now, "yyyyMMdd HH:mm:ss")) & ")"
            objDO.ExecuteSQL(strSql)
            MessageBox.Show("Loyalty Redemption Per Dollar Modified", "New Record", MessageBoxButtons.OK)
    ...
    Private Sub txtCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCode.TextChanged
        Try
            ...
            sCode = "Select Membertype, Points from LoyaltyRedPerDollar where MemberType=" & objDO.SafeSQL(txtCode.Text)
    ...
    Private Sub btnCustNo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCustNo.Click
        clear()
        RaiseEvent LoadDataForm(Windows.Forms.Application.StartupPath & "\DataList.dll", "DataList.MemberType", "SalesPromo.LoyaltyRedemptionPerDollar", "txtCode", 0, 0)
    End Sub
