...
Public Class LoyaltyByItem
    ...
    Private Sub LoyaltyByItem_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing

    End Sub

    Private Sub LoyaltyByItem_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub SaveLoyalty()
        ...
        objDO.ExecuteSQL("Delete from LoyaltyByItem where MemberType=" & objDO.SafeSQL(txtCode.Text))
        Try
            For cnt = 0 To dgvCustomerPrice.Rows.Count() - 2
                If dgvCustomerPrice.Item(0, cnt).Value = "" Then Exit For
                strSql = "Insert into LoyaltyByItem(MemberType, ItemNo, UOM, Points, DTG) Values (" & objDO.SafeSQL(txtCode.Text) & "," & objDO.SafeSQL(dgvCustomerPrice.Item(0, cnt).Value.ToString) & "," & objDO.SafeSQL(dgvCustomerPrice.Item(2, cnt).Value.ToString) & "," & dgvCustomerPrice.Item(3, cnt).Value & "," & objDO.SafeSQL(Format(DateTime.Now, "yyyyMMdd HH:mm:ss")) & ")"
                'MsgBox(strSql)
                objDO.ExecuteSQL(strSql)
            Next
            MessageBox.Show("Loyalty By Item Modified", "New Record", MessageBoxButtons.OK)
    ...
    Private Sub btnCustNo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCustNo.Click
        ' txtCode.Text = ""
        RaiseEvent LoadDataForm(Windows.Forms.Application.StartupPath & "\DataList.dll", "DataList.MemberType", "SalesPromo.LoyaltyByItem", "txtCode", 0, 0)
    End Sub
    ...
    Private Sub txtCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCode.TextChanged
        Try
            ...
            sCode = "Select LoyaltyByItem.ItemNo, Description, UOM, LoyaltyByItem.Points from Item, LoyaltyByItem where Item.ItemNo=LoyaltyByItem.ItemNo and MemberType=" & objDO.SafeSQL(txtCode.Text)
    ...
    Private Function IsItemExistsinCustProd(ByVal sItemNo As String) As Boolean
        Dim dtr As SqlDataReader
        Dim bAns As Boolean
        dtr = objDO.ReadRecord("Select ItemNo from CustProd where ItemNo = " & objDO.SafeSQL(sItemNo) & " and CustID=" & objDO.SafeSQL(txtCode.Text))
        bAns = dtr.Read
        dtr.Close()
        Return bAns
    End Function
    Private Function IsItemExistsinItemPr(ByVal sItemNo As String) As Boolean
        Dim dtr As SqlDataReader
        Dim bAns As Boolean
        dtr = objDO.ReadRecord("Select ItemNo from ItemPr where ItemNo = " & objDO.SafeSQL(sItemNo) & " and PriceGroup=" & objDO.SafeSQL(txtCode.Text))
        bAns = dtr.Read
        dtr.Close()
        Return bAns
    End Function
    ...
    Private Sub dgvCustomerPrice_CellMouseDown1(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvCustomerPrice.CellMouseDown
        ...
        If e.ColumnIndex = 0 Then
            If dgvCustomerPrice.Item(0, e.RowIndex).Value = "" Then
                ...
                'End If
                RaiseEvent LoadDataForm(Windows.Forms.Application.StartupPath & "\DataList.dll", "DataList.ItemListMultiple", "SalesPromo.LoyaltyByItem", "dgvCustPrice", 0, 0)
            End If
        End If
    End Sub
