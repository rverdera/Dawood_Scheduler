...
Public Class Inventory
    ...
    Public Sub loadLocation()
        ...
        While dtr.Read()
            aLoc.Add(New ComboValues(dtr("Code").ToString, dtr("Name").ToString))
        End While
        dtr.Close()
    ...
    Private Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        If cmbLocation.Text = "ALL" Then
            ExecuteSQL("Update InventoryRep set OrdQty = Qty from (Select Customer.Location, OrdItem.ItemNo, sum(Qty) * Uom.BaseQty as Qty from OrderHdr, OrdItem, Uom, Customer where " & _
            "OrderHdr.CustID = Customer.CustNo and OrderHdr.OrdNo = OrdItem.OrdNo and OrdItem.Uom = Uom.Uom and Uom.ItemNo = OrdItem.ItemNo and OrderHdr.OrdDt between " & SafeSQL(Format(dtpFromDate.Value, "yyyyMMdd")) & " and " & SafeSQL(Format(dtpToDate.Value, "yyyyMMdd 23:59:59")) & " Group by Customer.Location, OrdItem.ItemNo, Uom.BaseQty) " & _
            "A where A.ItemNo = InventoryRep.ItemID and A.Location = InventoryRep.Location")
        Else
            ExecuteSQL("Update InventoryRep set OrdQty = Qty from (Select Customer.Location, OrdItem.ItemNo, sum(Qty) * Uom.BaseQty as Qty from OrderHdr, OrdItem, Uom, Customer where " & _
            "Customer.Location = " & SafeSQL(cmbLocation.SelectedValue) & " and OrderHdr.CustID = Customer.CustNo and OrderHdr.OrdNo = OrdItem.OrdNo and OrdItem.Uom = Uom.Uom and Uom.ItemNo = OrdItem.ItemNo and OrderHdr.OrdDt between " & SafeSQL(Format(dtpFromDate.Value, "yyyyMMdd")) & " and " & SafeSQL(Format(dtpToDate.Value, "yyyyMMdd 23:59:59")) & " Group by OrdItem.ItemNo, Uom.BaseQty, Customer.Location) " & _
            "A where A.ItemNo = InventoryRep.ItemID and A.Location = InventoryRep.Location")
        End If
        
        Dim strSql As String = "SELECT * from InventoryRep order by ItemID"
        ExecuteReport(strSql, "InventoryRep")
    End Sub
