...
Public Class GoodsCheckINOUT
    ...
    Private Sub btnPrint_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        ExecuteSQL(strDel)
        dtr = ReadRecord("SELECT ItemNo, Description, BaseUOM FROM Item where Active=1 and ToPDA=1 order by DisplayNo")
        While dtr.Read = True
        ...
        For i = 0 To ArList.Count - 1
            ...
            'Uom,  "," & SafeSQL(dtr("Uom")) &
            If dIss = 0 And dTop = 0 And Actual = 0 Then Continue For
            Dim insSql As String = "Insert into GoodsCheck(DocNo,  ItemNo,ItemName, Uom, Issue, Topup, ActualReturn, AgentID, Date) values (''," & SafeSQL(aPr.ItemCode) & "," & SafeSQL(aPr.Desc) & "," & SafeSQL(aPr.UOM) & "," & dIss & "," & dTop & "," & Actual & "," & SafeSQL(sLocation) & ", '" & Format(dSyncDate, "yyyyMMdd HH:mm:ss") & "')"
            ExecuteSQL(insSql)
            'DisconnectAnotherDB()
        Next
        dtr.Close()
        Dim strSql As String = "Select * from GoodsCheck"
